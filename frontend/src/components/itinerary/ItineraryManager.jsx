          ...
          <article key={day.id} className="itinerary-day-card">
            <div className="itinerary-day-header" onClick={() => setExpandedDays((current) => ({ ...current, [day.id]: !current[day.id] }))}>
              <div>
                ...
                <span className="itinerary-day-date">{new Date(day.date).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' })}</span>
              </div>
              <div className="itinerary-day-actions" onClick={(event) => event.stopPropagation()}>
                <Link to={`/activity-scheduler/${day.id}`} className="secondary-button compact-button" style={{ textDecoration: 'none' }}>Manage Activities</Link>
                {canModifyDay && (
                  <>
