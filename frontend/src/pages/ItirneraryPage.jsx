...
function ItineraryPage() {
  ...
  const loadTrip = useCallback(async () => {
    ...
    try {
      ...
      if (tripId) {
        setTrip(null)
        setTrip(await tripApi.getTrip(tripId))
      } else {
        const allTrips = await tripApi.getAllTrips()
        setTrips(allTrips)
        if (allTrips.length > 0) {
          navigate(`/itinerary/${allTrips[0].id}`, { replace: true })
        }
      }
    } catch (err) {
        ...
        <div className="dashboard-content">
          ...
          <main className="dashboard-main">
            <section className="section-card">
              {loading || (tripId && !trip) ? <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-secondary)' }}>Loading itinerary...</div> : error ? (
